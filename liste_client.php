...
<ul class="nav nav-pills nav-justified">
  ...
  <li class="active"><a href="liste_client.php" >listes des clients</a></li>
  ...
  <tbody>
    ...
    <?php 
    include("par.php");
    $records = $bdd->prepare('SELECT * FROM  user');
      if ($records->execute()) {
        $count=1;
...
echo '<tr> <th scope="row">'.$count.'</th>
      ...
      <td>'.$tele.'</td>
      <td>'.$com.'</td>
      <td><i class="glyphicon glyphicon-remove btn" style="color:red;" onclick="delet_user('.$id.');"></i></td> </tr>';
      $count++;
        }
...
<?php  
function nbr_comm($id){
  include("par.php");
  $etat=1;
  $records = $bdd->prepare('SELECT count(id_comm) FROM  commande WHERE etat_comm=:nbr AND id_client=(
SELECT id_user FROM user WHERE id_user=:id 
    )');
  $records->bindParam(':id',$id);
  $records->bindParam(':nbr',$etat);
   $records->execute();
            $rowCount = $records->fetchColumn(0);
            return $rowCount;
}
function email_lue(){
   include("par.php");
